...
class PianorollEncoderDecoder(object):
  ...
  def encode(self, sequence, embeddings):
    """Encode sequence of slices into sequence of embeddings."""
    sequence = list(filter(lambda slice: slice != [], sequence))

    if self.separate_instruments:
      roll = np.zeros((len(sequence), self.embedding_dim, self.num_instruments))
    else:
      roll = np.zeros((len(sequence), self.embedding_dim, 1))

    for i, slice in enumerate(sequence):
      roll[i,:,0] = embeddings[np.mod(hash(str(set(slice))), self.vocab_size)]
    # import pickle
    # with open('roll_test', 'wb') as f:
    #   pickle.dump(roll, f)
    return roll
  ...
  # def decode_to_midi(self, pianoroll):
  #   """Decodes pianoroll into midi or into either original or integer slice hashes."""
  #   # NOTE: Assumes four separate instruments ordered from high to low.
  #
  #   return midi_data
