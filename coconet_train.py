...
flags.DEFINE_string('embedding_fname', None,
                    'Path to the slice embeddings.')
...
def main(unused_argv):
  ...
  train_data = lib_data.get_dataset(FLAGS.data_dir, hparams, 'train', FLAGS.embedding_fname)
  valid_data = lib_data.get_dataset(FLAGS.data_dir, hparams, 'valid', FLAGS.embedding_fname)
  ...
  # with open('train_data_pre-update', 'wb') as f:
  ...
  #     pickle.dump(train_data, f)
  # with open('valid_data', 'wb') as f:
  #     pickle.dump(valid_data, f)
  # Save hparam configs.
  # logdir = os.path.join(FLAGS.logdir, hparams.log_subdir_str)
  logdir = os.path.join(FLAGS.logdir, hparams.log_subdir_str[:20]+'_embed_test')
  ...
  with tf.Graph().as_default():
    ...
    tracker = Tracker(
        ...
        decay_op=m.decay_op,
        save_path=os.path.join(FLAGS.logdir, hparams.log_subdir_str[:20]+'_embed_test',
                               'best_model.ckpt'))
...
def _hparams_from_flags():
  ...
  # params = dict((key, getattr(FLAGS, key)) for key in keys)
  # params['min_pitch'] = 0
  # params['max_pitch'] = 19
  # hparams = lib_hparams.Hyperparameters(**params)
  return hparams
