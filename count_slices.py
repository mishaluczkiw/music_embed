...
# def generate_slices2(start, end, file_prefix):
...
#                 counts = pd.Series(slices).value_counts()
#                 slice_counts = slice_counts.add(counts, fill_value=0)
#                 # with open("%s_%d-%d" % (file_prefix, start, end), "ab") as f:
#                 #     pickle.dump(slices, f)
#             except:
#                 with open("bad_midis.txt", "a", encoding='utf-8') as f:
...
def combine_all_slice_counts(slice_files, threshold=10):
    slices = pd.Series()
...
plot_slices_bar(slices, filename='slices_aaa.html')
